...
# # from server.utils.azure_openai_client import get_chat_client, get_chat_deployment
...
# # ) -> str:
# #         """
# #         Decide FM Origin based ONLY on where the Failure Mode text appears.
# #         Priority: KB → SPRLL → PRD → Repair. If no match → "AI".

# #         Fluid, data-driven matching:
# #         - Uses source-provided FM-like field (payload['match_text_fm']) when available.
# #         - Builds a dynamic generic-token list from the batch (no hard-coded stopwords).
# #         - KB is strict: full FM substring match only (no token fallback).
# #         - SPRLL/Repair: substring OR (≥2 rare tokens AND coverage ≥ 40%).
# #         - PRD: substring OR (≥2 rare tokens AND coverage ≥ 30%) — PRDs are terse.
# #         """
# #         import re
# #         from collections import Counter
...
# #         def get_text(c) -> str:
# #             # Prefer source-specific FM field if available
# #             p = get_payload(c) or {}
...
# #         fm_norm = norm(fm_raw)
# #         fm_tokens_all = [t for t in tokenize(fm_norm) if len(t) >= 5]
# #         fm_token_set_all = set(fm_tokens_all)

# #         # ---------- dynamic generic terms (document-frequency based) ----------
# #         all_chunks = (kb_chunks or []) + (sprll_chunks or []) + (prd_chunks or []) + (rd_chunks or [])
# #         if not all_chunks:
# #             return "AI"

# #         MIN_TOKEN_LEN = 5
# #         GENERIC_DF_FRAC = 0.12  # tokens appearing in ≥12% of chunks are considered "generic"
...
# #         generic = {t for t, c in df.items() if (c / total_docs) >= GENERIC_DF_FRAC}

# #         # FM tokens we consider for rarity-based fallback
# #         fm_tokens_rare = [t for t in fm_token_set_all if t not in generic]
# #         fm_tokens_rare_set = set(fm_tokens_rare)

# #         # ---------- matchers ----------
# #         def has_substring(chunks: List[Dict]) -> bool:
# #             if not fm_norm or not chunks:
# #                 return False
# #             for c in chunks:
# #                 if fm_norm in get_text(c):
# #                     return True
# #             return False

# #         def rare_token_fallback(chunks: List[Dict], min_hits: int, min_coverage: float) -> bool:
# #             """
# #             Require:
# #             - at least `min_hits` distinct rare FM tokens present in the chunk text, AND
# #             - coverage (matched_rare_tokens / total_rare_FM_tokens) >= min_coverage
# #             Evaluated across chunks; any single chunk satisfying the condition passes.
# #             """
...
# #                 if len(hits) >= min_hits and (len(hits) / total_rare) >= min_coverage:
# #                     return True
# #             return False

# #         # ---------- source-specific rules ----------
# #         # KB: strict substring only
# #         if has_substring(kb_chunks):
# #             return "kb"

# #         # SPRLL: substring OR rare-token fallback (≥2 hits & ≥40% coverage)
# #         if has_substring(sprll_chunks) or rare_token_fallback(sprll_chunks, min_hits=2, min_coverage=0.40):
# #             return "sprll"

# #         # PRD: substring OR rare-token fallback (≥2 hits & ≥30% coverage) — slightly looser
# #         if has_substring(prd_chunks) or rare_token_fallback(prd_chunks, min_hits=2, min_coverage=0.30):
# #             return "prds"

# #         # Repair: substring OR rare-token fallback (≥2 hits & ≥40% coverage)
# #         if has_substring(rd_chunks) or rare_token_fallback(rd_chunks, min_hits=2, min_coverage=0.40):
# #             return "repair_data"
...
class ContextAgent:
    ...
    rd_chunks: List[Dict],
        ...
        # === Phase 1: Repair Data (slightly controlled) ===
        if (has_substring(rd_chunks) or rare_token_fallback(rd_chunks, 20, 0.18) or direct_semantic_match(rd_chunks)) \
                and not rare_token_fallback(kb_chunks, 3, 0.25):
            return "repair_data"

        # === Phase 2: SPRLL ===
        if has_substring(sprll_chunks) or rare_token_fallback(sprll_chunks, 4, 0.12):
            return "sprll"

        # === Phase 3: PRDs ===
        if has_substring(prd_chunks) or rare_token_fallback(prd_chunks, 3, 0.25):
            return "prds"


        # === Phase 4: KB (slightly boosted) ===
        if has_substring(kb_chunks) or rare_token_fallback(kb_chunks, 5, 0.35):
            return "kb"
